        ...
        <div class="order-list new-orders">
          ...
          >
            ...
            <div class="order-actions" v-if="order.id">
              <el-button type="success" @click="handleAccept(order.id!)"
                >接单</el-button
              >
              <el-button type="danger" @click="handleReject(order.id!)"
                >拒单</el-button
              >
            </div>
          </el-card>
...
watch(message, newMessage => {
  if (
    ...
    'type' in newMessage &&
    newMessage.type === 'NEW_ORDER'
  ) {
    const order = newMessage.payload as Order
    if (
      order.business?.id === business.value?.id &&
      !newOrders.value.some(o => o.id === order.id)
    ) {
