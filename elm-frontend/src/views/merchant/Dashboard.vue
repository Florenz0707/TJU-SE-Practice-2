<template>
  <div v-loading="loading" class="dashboard-container">
    ...
    <el-row v-if="!showNoBusinessMessage" :gutter="20">
      <!-- New Orders Column -->
      <el-col :span="12">
        <h3>新进订单</h3>
        <div class="order-list new-orders">
          ...
          >
            ...
            <div v-if="order.id" class="order-actions">
              <el-button type="success" @click="handleAccept(order)"
                >接单</el-button
              >
              <el-button type="danger" @click="handleReject(order)"
                >拒单</el-button
              >
            </div>
          </el-card>
          ...
          >
            <div class="order-content">
              ...
              <p>
                ...
                <el-tag
                  :type="
                    getOrderStatusInfo(order.orderState as OrderStatus).type
                  "
                >
            ...
            <div v-if="order.id" class="order-actions">
              <el-button
                v-if="order.orderState === OrderStatusEnum.ACCEPTED"
                @click="updateStatus(order, OrderStatusEnum.DELIVERY)"
                >开始配送</el-button
              >
              <el-button
                v-if="order.orderState === OrderStatusEnum.DELIVERY"
                @click="updateStatus(order, OrderStatusEnum.COMPLETE)"
                >订单完成</el-button
              >
            </div>
          </el-card>
  ...
  try {
    ...
    if (businessId) {
      ...
      if (response.success) {
        ...
        newOrders.value = allOrders.filter(
          (o: Order) => o.orderState === OrderStatusEnum.PAID,
        );
        inProgressOrders.value = allOrders.filter(
          (o: Order) =>
            o.orderState === OrderStatusEnum.ACCEPTED ||
            o.orderState === OrderStatusEnum.DELIVERY,
        );
      } else {
...
const showNoBusinessMessage = computed(
  () => !selectedBusiness.value && !loading.value,
);
