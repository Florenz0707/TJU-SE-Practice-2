<template>
  <div>
    <el-page-header title="返回订单列表" @back="goBack">
      <template #content>
    ...
    <el-card v-if="order" class="order-summary-card">
      ...
      <el-descriptions :column="2" border>
        <el-descriptions-item label="餐厅">{{
          order.business?.businessName ?? "暂无"
        ...
        <el-descriptions-item label="下单日期">{{
          order.orderDate ? new Date(order.orderDate).toLocaleString() : "暂无"
        }}</el-descriptions-item>
        <el-descriptions-item label="配送地址" :span="2">{{
          order.deliveryAddress?.address ?? "暂无"
        }}</el-descriptions-item>
        <el-descriptions-item label="订单金额" :span="2"
          >¥{{ (order.orderTotal ?? 0).toFixed(2) }}</el-descriptions-item
        >
        <el-descriptions-item
          v-if="order.voucherDiscount && order.voucherDiscount > 0"
          label="优惠券折扣"
          :span="2"
        >
        ...
        <el-descriptions-item
          v-if="order.pointsUsed && order.pointsUsed > 0"
          label="积分抵扣"
          :span="2"
        >
          {{ order.pointsUsed }}积分 (-¥{{
            (order.pointsDiscount ?? 0).toFixed(2)
          }})
        </el-descriptions-item>
        <el-descriptions-item
          v-if="order.walletPaid && order.walletPaid > 0"
          label="钱包支付"
          :span="2"
        >
