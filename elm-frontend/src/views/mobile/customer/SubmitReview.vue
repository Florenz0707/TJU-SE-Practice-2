<template>
  <div class="mobile-submit-review-page">
    <el-page-header content="评价订单" @back="goBack"></el-page-header>
      ...
      <el-card>
        ...
        <el-form ref="formRef" :model="reviewForm" label-position="top">
          <el-form-item
            label="评分"
            prop="stars"
            :rules="{ required: true, message: '请给出一个评分' }"
          >
...
onMounted(async () => {
  ...
  try {
    ...
    error.value =
      (err instanceof Error ? err.message : String(err)) || "获取订单失败";
  } finally {
    loading.value = false;
  }
});
...
const submitReview = async () => {
  ...
  try {
    ...
    ElMessage.error(
      (err instanceof Error ? err.message : String(err)) || "提交评价失败",
    );
  } finally {
