        ...
        <div v-for="rule in rules" :key="rule.id" class="rule-item">
          <div class="rule-info">
            <div class="rule-header">
              ...
              <el-tag
                size="small"
                :type="rule.isEnabled ? 'success' : 'info'"
                >{{ rule.isEnabled ? "启用" : "禁用" }}</el-tag
              >
            </div>
            <div class="rule-detail">
              ...
              <span class="expiry"
                >有效期:
                {{
                  rule.expireDays === -1 ? "永久" : rule.expireDays + "天"
                }}</span
              >
            </div>
          </div>
...
const fetchRules = async () => {
  try {
    ...
    ElMessage.error("获取规则失败: " + (e as Error).message);
  }
};
