  ...
  <div class="user-management">
    ...
    <DataTable :columns="columns" :data="filteredUsers">
      <template #actions="{ row }">
        <el-button size="small" @click="handleEdit(row as User)"
          >编辑</el-button
        >
        <el-button
          size="small"
          type="warning"
          @click="handlePasswordChange(row as User)"
          >修改密码</el-button
        >
        <el-button size="small" type="danger" @click="handleDelete(row as User)"
          >删除</el-button
        >
      </template>
    </DataTable>
  ...
  await createUserFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.error(
          "创建用户失败: " + (error instanceof Error ? error.message : ""),
        );
      }
    }
  });
