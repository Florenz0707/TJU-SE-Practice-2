  ...
  <div class="shop-management">
    ...
    <el-dialog v-model="detailsVisible" title="申请详情" width="50%">
      <div v-if="selectedApplication">
        <p>
          <strong>店铺名称:</strong>
          {{ selectedApplication.business.businessName }}
        </p>
        <p>
          <strong>店铺地址:</strong>
          {{ selectedApplication.business.businessAddress }}
        </p>
        <p>
          <strong>店铺简介:</strong>
          {{ selectedApplication.business.businessExplain }}
        </p>
        <p>
          <strong>起送价:</strong> {{ selectedApplication.business.startPrice }}
        </p>
        <p>
          <strong>配送费:</strong>
          {{ selectedApplication.business.deliveryPrice }}
        </p>
        <p>
          <strong>申请说明:</strong>
          {{ selectedApplication.applicationExplain }}
        </p>
      </div>
      <template #footer>
...
const updateApplicationStatus = async (id: number, state: number) => {
  try {
    await updateBusinessApplication(id, {
      applicationState: state as ApplicationState,
    });
