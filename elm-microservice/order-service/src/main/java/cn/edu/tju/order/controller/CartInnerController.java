...
@RestController
@RequestMapping("/api/inner/order/cart")
@Tag(name = "购物车内部接口", description = "order-service 购物车域内部接口")
public class CartInnerController {
  ...
  public HttpResult<CartSnapshotVO> createCart(@RequestBody CreateCartRequest request) {
    ...
    try {
      CartSnapshotVO created =
          cartInternalService.createCart(
              new CartInternalService.CreateCartCommand(
                  request.getFoodId(),
  ...
  public HttpResult<CartSnapshotVO> getCartById(
      @Parameter(description = "购物车ID", required = true) @PathVariable("cartId") Long cartId) {
  ...
  public HttpResult<List<CartSnapshotVO>> getCartsByCustomerId(
  ...
  public HttpResult<List<CartSnapshotVO>> getCartsByBusinessAndCustomerId(
      @Parameter(description = "商家ID", required = true) @PathVariable("businessId") Long businessId,
      @Parameter(description = "用户ID", required = true) @PathVariable("customerId")
          Long customerId) {
    return HttpResult.success(
        cartInternalService.getCartsByBusinessAndCustomerId(businessId, customerId));
  }

  @PostMapping("/{cartId}/quantity")
  @Operation(summary = "更新购物车数量", description = "更新购物车项数量")
  public HttpResult<CartSnapshotVO> updateCartQuantity(
      @Parameter(description = "购物车ID", required = true) @PathVariable("cartId") Long cartId,
    ...
    try {
      return HttpResult.success(
          cartInternalService.updateCartQuantity(cartId, request.getQuantity()));
    } catch (Exception e) {
      return HttpResult.failure(ResultCodeEnum.SERVER_ERROR, e.getMessage());
    }
  }
  ...
  public HttpResult<Boolean> deleteCart(
      @Parameter(description = "购物车ID", required = true) @PathVariable("cartId") Long cartId) {
    try {
