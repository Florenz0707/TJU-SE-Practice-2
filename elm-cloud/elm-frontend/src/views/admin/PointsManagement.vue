    ...
    <el-card>
      ...
      <el-table :data="rules" style="width: 100%">
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button size="small" @click="openRuleDialog(scope.row)"
              >编辑</el-button
            >
            <el-button
              size="small"
...
async function fetchRules() {
  try {
    ...
    ElMessage.error(
      "获取积分规则失败: " + (error instanceof Error ? error.message : ""),
    );
  }
}
...
async function saveRule() {
  try {
    ...
    ElMessage.error(
      "保存规则失败: " + (error instanceof Error ? error.message : ""),
    );
  }
}
...
async function handleDelete(id: number) {
  try {
    ...
    ElMessage.error(
      "删除规则失败: " + (error instanceof Error ? error.message : ""),
    );
  }
}
