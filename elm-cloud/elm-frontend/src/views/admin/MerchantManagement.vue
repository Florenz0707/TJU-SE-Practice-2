    ...
    <el-row :gutter="20" style="margin-bottom: 20px">
      ...
      <el-col :span="8">
        <el-select v-model="statusFilter" placeholder="按状态筛选" clearable>
          <el-option
            label="待处理"
            :value="ApplicationState.UNDISPOSED"
          ></el-option>
          <el-option
            label="已批准"
            :value="ApplicationState.APPROVED"
          ></el-option>
          <el-option
            label="已拒绝"
            :value="ApplicationState.REJECTED"
          ></el-option>
        </el-select>
      </el-col>
    </el-row>
      ...
      <template #actions="{ row }">
        <div v-if="row.applicationState === ApplicationState.UNDISPOSED">
          <el-button
            size="small"
            type="success"
            @click="handleUpdate(row, ApplicationState.APPROVED)"
            >批准</el-button
          >
          <el-button
            size="small"
            type="danger"
            @click="handleUpdate(row, ApplicationState.REJECTED)"
            >拒绝</el-button
          >
        </div>
...
const fetchApplications = async () => {
  try {
    ...
    ElMessage.error(
      "获取申请列表失败: " + (error instanceof Error ? error.message : ""),
    );
  }
};
...
const filteredApplications = computed(() => {
  return applications.value.filter((app) => {
    ...
    const statusMatch =
      ...
      app.applicationState === statusFilter.value;
...
const handleUpdate = async (
  ...
  try {
    ...
    ElMessage.error(
      "操作失败: " + (error instanceof Error ? error.message : ""),
    );
  }
};
</script>
