    ...
    <el-card class="wallet-summary">
      ...
      <div v-else class="balance-container">
        ...
        <div class="wallet-info">
          ...
          <span v-if="wallet?.lastWithdrawalAt" class="withdrawal-info"
            >上次提现:
            {{ new Date(wallet.lastWithdrawalAt).toLocaleString() }}</span
          >
        </div>
...
onMounted(async () => {
  ...
  if (!walletStore.wallet) {
    ...
    try {
      ...
      ElMessage.error(
        "无法创建钱包，请联系管理员: " + (e instanceof Error ? e.message : ""),
      );
    }
  }
...
const confirmRecharge = async () => {
  ...
  try {
    ...
    ElMessage.error("充值失败: " + (e instanceof Error ? e.message : ""));
  }
};
...
const confirmWithdraw = async () => {
  ...
  try {
    ...
    ElMessage.error("提现失败: " + (e instanceof Error ? e.message : ""));
  }
};
</script>
