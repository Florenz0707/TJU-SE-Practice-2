...
- 当前已接入 Config Client 的服务：`gateway`、`order-service`、`user-service`
- 当前客户端通过 `bootstrap.yml` 中的 `spring.cloud.config.uri=http://config-server:8888` 直连配置中心

> 说明：当前 `elm-cloud` 不是课件中的“Git 配置仓库 + Config Server 集群 + 全量服务接入”模式，而是采用了更适合本地 Docker 演示的替代实现：Config Server 负责统一提供配置，配置源来自宿主机挂载目录，客户端先在少量样例服务上分批接入。
...
当前 `elm-cloud` 已经部分落地 Bus：

- `docker-compose.yml` 中已经部署了 RabbitMQ。
- `config-server`、`order-service`、`user-service` 已引入 `spring-cloud-starter-bus-amqp`。
- `config-server` 已暴露 `bus-refresh`、`bus-env` 端点。
- `order-service`、`user-service` 已暴露 `bus-refresh` / `refresh` 相关端点。

因此：**Bus 的基础设施链路已经落地启用，但覆盖范围不是全量服务。**

同时也要注意两点：

- 当前代码中没有发现 `@RefreshScope` 的实际业务用例，因此“可见的业务配置热更新演示点”还不完整。
- `gateway` 虽然接入了 Config Client，但没有接入 Bus；其余业务服务目前也没有接入 Bus。

### 9.3 如果要在 elm-cloud 上落地（演进建议）

- 把 `gateway` 与其余业务服务逐步迁移到 Config Client + Bus。
- 在需要动态刷新的配置 bean 上使用 `@RefreshScope`，补一个可观测的演示接口。
- 如果要回到课件标准方案，可将 native 模式切换为 Git 模式，并扩成 Config Server 集群。
